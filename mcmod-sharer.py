def main():
    ...
    export_parser.add_argument('--game-version', '-gv', action='store', default=[''], nargs=1, metavar=(
        '<game_version>',), help='Minecraft version.', required=True)
    
    export_parser.add_argument('--mod-loader', '-ml', action='store', default=[''], nargs="*", metavar=(
        '<mod_loader>',), help='Minecraft mod loader.', required=True)
    
    export_parser.add_argument('archive_path', help='Path to the archive you are pulling mods list from.')
    
    export_parser.add_argument('--file', '-f', action='store', default=[''], nargs=1, metavar=(
        '<file_path>',), help='Write mod list to a file.')
    
    export_parser.add_argument('--copy', '-c', action='store_true', default=False,
